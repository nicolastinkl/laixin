  ...
  if( self ) {
    ...
    if (self.cancelEnabled) {
      ...
      if ([self.cancelButtonTitle length] > 0 ) {
        ...
        [cancelButton setTitle:NSLocalizedStringWithDefaultValue(@"OverlayView cancel button title", nil, [NSBundle mainBundle], @"关闭", @"关闭") forState:UIControlStateNormal];
      }
...
- (void)drawRect:(CGRect)rect {
  ...
  if (displayedMessage == nil) {
      self.displayedMessage = NSLocalizedStringWithDefaultValue(@"OverlayView displayed message", nil, [NSBundle mainBundle],@"将二维码放入框内,即可自动扫描",@"将二维码放入框内,即可自动扫描");
                                                              //@"Place a barcode inside the viewfinder rectangle to scan it.", @"Place a barcode inside the viewfinder rectangle to scan it.");
      //将二维码放入框内,即可自动扫描
  }

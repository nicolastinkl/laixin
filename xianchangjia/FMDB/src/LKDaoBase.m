//
//
//  Created by JIJIA &&&&& y h on 12-10-9.
...
-(id)initWithDBQueue:(FMDatabaseQueue*)queue tableName:(NSString *) name model:(Class) modelName
{
    ...
    if (self)
    {
		...
		self.tableName = name;
		self.classType = modelName;
		
		//获取绑定的 Model 并 保存 Model 的属性信息
		NSDictionary* dic  = [modelName getPropertys];
		...
		dispatch_once(&onceToken, ^{
			onceCreateTable = [[NSMutableDictionary  alloc]initWithCapacity:120];
		});
		NSString* className = name;//NSStringFromClass(self.class);
...
-(id)initWithDBQueue:(FMDatabaseQueue *)queue
{
	...
	if (self)
	{
		...
		//获取绑定的 Model 并 保存 Model 的属性信息
		NSDictionary* dic  = [self.classType getPropertys];
		NSArray* pronames = [dic objectForKey:@"name"];
		NSArray* protypes = [dic objectForKey:@"type"];
		self.propertys = [NSMutableDictionary dictionaryWithObjects:protypes forKeys:pronames];
		for (int i =0; i<pronames.count; i++) {
		...
		dispatch_once(&onceToken, ^{
			onceCreateTable = [[NSMutableDictionary  alloc]initWithCapacity:1];
		});
    ...
    [bindingQueue inDatabase:^(FMDatabase* db)
     {
         ...
         SLog(@"create table %@",self.tableName);
     }];
}
    ...
    [bindingQueue inDatabase:^(FMDatabase* db)
     {
         ...
         [self executeResult:set block:block];
		 if ([db hadError]) {
			 SLog(@"Err %d: %@", [db lastErrorCode], [db lastErrorMessage]);
		 }
     }];
}
...
- (void)executeResult:(FMResultSet *)set block:(void (^)(NSArray *))block
{
    ...
    while ([set next]) {
        NSObject<LKModelBaseInteface>* bindingModel = [[self.classType alloc] init];
...
-(void)updateToDBWithSql:(NSString *) update callback:(void (^)(BOOL))block
{
	//examply  update: unreadmessage = 2 where rouid = 1
	[bindingQueue inDatabase:^(FMDatabase* db){
		NSString* updateSQL;
		updateSQL = [NSString stringWithFormat:@"update %@ set %@ ",self.tableName,update];
		...
		if(execute == NO)
		{
			NSLog(@"database update fail %@ ",self.tableName);
		}

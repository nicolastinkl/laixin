...
//		label.shadowColor = [UIColor colorWithWhite:0.900 alpha:0.820];
//		label.shadowOffset = CGSizeMake(0.0f, .5f);
		...
		view.frame = CGRectMake(20, 12, 20.0f, 20.0f);
...
- (void)setState:(PWLoadMoreState)aState{
	...
	switch (aState) {
		case PWLoadMoreNormal:
			...
			_statusLabel.text = @"点击加载更多";// NSLocalizedString(@"Load More", @"Load More items");
		...
		case PWLoadMoreLoading:
			...
			_statusLabel.text = @"正在加载中...";// NSLocalizedString(@"Loading...", @"Loading items");
			[_activityView startAnimating];
			
			break;
		case PWLoadMoreDone:
            [self removeTarget:self action:@selector(callDelegateToLoadMore) forControlEvents:UIControlEventTouchUpInside];
			_statusLabel.text =@"已全部加载" ;// NSLocalizedString(@"No More", @"There is no more item");
			[_activityView stopAnimating];
...
- (void)callDelegateToLoadMore {
    if (_state == PWLoadMoreNormal) {
        if ([_delegate respondsToSelector:@selector(pwLoadMoreTableDataSourceIsLoading)]) {
            if ([_delegate pwLoadMoreTableDataSourceIsLoading]) {
                [self removeTarget:self action:@selector(callDelegateToLoadMore) forControlEvents:UIControlEventTouchUpInside];
                _statusLabel.text = @"已全部加载";// NSLocalizedString(@"Not available now...", @"Wait until it's safe to load more");
